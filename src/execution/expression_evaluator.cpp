...
Value ExpressionEvaluator::EvaluateArithmeticOp(const Value& left, const Value& right, BinaryOpExpression::OpType op) {
    ...
    auto to_double = [](const Value& val) -> double {
        ...
        throw ExecutionException("Cannot convert value to numeric for arithmetic operation");
    };
    ...
    try {
        ...
        // 如果原来都是整数，尝试返回整数结果
        if ((std::holds_alternative<int32_t>(left) || std::holds_alternative<int64_t>(left)) &&
            (std::holds_alternative<int32_t>(right) || std::holds_alternative<int64_t>(right)) &&
            op != BinaryOpExpression::OpType::DIVIDE) {
